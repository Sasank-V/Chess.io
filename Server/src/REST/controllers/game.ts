...
import { AddMoveRequest, AddMoveResponse, CreateGameRequest, CreateGameResponse, GameOverRequest, GameOverResponse } from "../types/game"
...
export const addMoveHandler: RequestHandler<{},AddMoveResponse,AddMoveRequest> = async (req,res) => {
    try {
...
export const gameOverHandler: RequestHandler<{},GameOverResponse,GameOverRequest> = async (req,res) => {
    try {
        const {gameId,winnerUsername,reason} = req.body;
        const game = await Game.findById(gameId);
        if(!game){
            res.status(404).json({
                success:false,
